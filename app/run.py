...
def tokenize(text):
    ...
    Tokenizes text data
    Args:
    text str: strings containing messages
    Returns:
...
def index():
    ...
    genre_counts = df.groupby('genre').count()['message'] # count messages based on genre
    genre_names = list(genre_counts.index)                # genre names
    cat_p = df[df.columns[4:]].sum()/len(df)              # proportion of each cat against whole df 
    cat_p = cat_p.sort_values(ascending = False)          # sorting values descending from left
    ...
    sorted_count_words = dict(sorted(Counter(words_with_repetition).items(),
                                         key=operator.itemgetter(1),
                                         reverse=True))   
    top, top_10 =0, {}

    for k,v in sorted_count_w.items():
        top_10[k]=v
        top+=1
        if top==10:
            break
